...
- 标签分两类，分别输出到 tags 和 companies 两个数组，不要混在一起：
  · tags：概念/主题标签（如"美联储""财报解读""半导体""技术分析"），
    ...
    标签；最多 6 个。
  · companies：文章提到的具体公司/股票代码/具名人物/具名产品，**也包括
    提到的大盘指数和板块/行业 ETF**（如"英伟达""TSLA""鲍威尔""ChatGPT"，
    以及"标普500""纳斯达克100""纳指""SOXX""费城半导体""IGV"这类），
    不算概念标签，不要塞进 tags；指数/ETF 用统一写法：标普500 写成
    "标普500"、纳斯达克100 写成"纳斯达克100"、费城半导体/SOX/SOXX 统一
    写成"SOXX"、软件行业ETF写成"IGV"，不要写成"标普"、"纳指"、"半导体
    指数"这类简称或同义词；若给了"已有公司库"同样优先复用同一写法（如
    已有"英伟达"就不要再写"NVIDIA"）；最多 6 个，没有就输出空数组。
输出 JSON：
...
· tags：概念/主题标签，若给了"已有标签库"，优先从中选择贴切的复用，不要
  ...
  最多新造 1 个新标签；最多 6 个。
· companies：文章提到的具体公司/股票代码/具名人物/具名产品，**也包括
  提到的大盘指数和板块/行业 ETF**（标普500、纳斯达克100、SOXX、IGV），
  统一写法：标普500/纳斯达克100/SOXX/IGV，不要写"标普""纳指""半导体
  指数"这类简称，不算概念标签，不要塞进 tags；若给了"已有公司库"同样
  优先复用同一写法；最多 6 个，没有就输出空数组。
